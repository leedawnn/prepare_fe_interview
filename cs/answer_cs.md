...
- HTTP는 상태가 없는(stateless) 프로토콜로서 데이터를 주고 받기 위한 각각의 데이터 요청이 서로 독립적으로 관리가 됩니다.(이전 데이터 요청과 다음 데이터 요청이 서로 관련x) (stateless 추가,,,)또한, HTTP는 암호화가 되지 않은 평문 데이터를 전송하는 프로토콜이기 때문에, 민감한 정보(비밀번호, 주민등록번호 등)를 주고 받으면 제3자가 정보를 조회할 수 있습니다. 이런 문제를 해결하기 위해 HTTP에 SSL을 적용하여 보안을 강화한 것이 HTTPS입니다. SSL에서는 대칭키 방식과 공개키 방식을 적절히 혼용하여 서버의 신원을 보증하고, 데이터 통신을 암호화합니다.
...
- 개인적으로 에러 방지를 위함이 가장 크다고 느꼈습니다. 자바스크립트의 경우 콘솔로 찍거나 브라우저 상으로 확인해야만 데이터 반환 결과를 알 수 있는데, 타입스크립트를 사용하여 미리 타입을 지정하면 오탈자를 알려주고, 잘못된 값이 들어오는 것을 막아줍니다. TS를 사용하지 않는다면 JSDoc으로 미리 타입을 명시해줘야 하는데 이것도 어마어마하게 귀찮은 일.
