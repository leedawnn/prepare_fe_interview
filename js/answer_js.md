...
- 타이머 🔥
  ...
  - 호출 스케쥴링이 무엇인가요?
    - 타이머 함수를 사용하여 명시적으로 호출하지 않고, 일정 시간 이후에 호출되도록 함수 호출을 예약하는 것 을 호출 스케쥴링이라고 합니다.
  - 타이머 함수에는 어떤 것들이 있나요?
...
- Ajax 🔥
  ...
  - Ajax가 뭔가요 어떤 것을 담당하고 있죠?
    - Ajax(Asynchronous JavaScript and XML)란 자바스크립트를 사용하여 ① 브라우저가 ② 서버에게 비동기 방식으로 데이터를 요청하고, 서버가 응답한 데이터를 수신하여 웹페이지를 동적으로 갱신하는 프로그래밍 방식 을 말합니다.
